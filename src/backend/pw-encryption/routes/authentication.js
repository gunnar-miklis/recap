...
//	* Authorization: process of determining what actions a user is allowed to access (guest, user, editor, admin, etc.)
...
router.post('/users/auth', async (req, res, next) => {
  ...
  try {
    ...
    const isVerified = bcrypt.compareSync(plainPassword, foundUser.password);
    if (!isVerified) {
