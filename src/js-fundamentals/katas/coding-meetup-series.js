...
function findAdmin(list, obj) {
  return list.filter((developer) => developer.language === obj && developer.githubAdmin === 'yes');
}
...
function countDevelopers3(list) {
  return list.filter(
    (developer) => developer.continent === 'Europe' && developer.language === 'JavaScript',
  ).length;
}
...
function getAverageAge(list) {
  return Math.round(list.reduce((sum, developer) => sum + developer.age, 0) / list.length);
}
