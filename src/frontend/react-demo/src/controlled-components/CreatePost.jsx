...
export default function CreatePost({ authors, postsCount, toggleIsVisible, getPosts }) {
  ...
  return (
    <>
      ...
      <form className='flex-col' method='POST' onSubmit={handleSubmit}>
        ...
        <select id='authors' name='authors' onChange={handleAuthorSelection} multiple required>
          {authors &&
            authors.map((author) => (
