...
(async () => {
	try {
		...
		if (
			updatedCampuses[0].students.length &&
			updatedCampuses[1].students.length
		) {
		...
		console.log(
			`${
				...
				createdCampuses.length
					? createdCampuses.length + ' campuses added!'
					: ''
			} ${
				areRelationsLinked
					? 'Students-Campus are linked: many-to-many!'
					: 'Without relations!'
			}`,
		);
	} catch (err) {
